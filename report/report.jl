...
# ╔═╡ dad89db9-c04b-476f-be4e-56828e48dbe6
# ╠═╡ show_logs = false
using LinearAlgebra,Distributions,StatsBase,Random,Printf,Plots

# ╔═╡ 65b55bfc-1e98-11ed-0839-93cf4d2116a5
begin
...
# ╔═╡ e93c7f45-0b8a-4f97-a60a-c8db05cd6cc1
md"
# Sequential Monte Carlo Demo

This module revolves around the `StateSpaceModel` which includes two main aspects, the model cahracteristics and the dimensions of the observation/transition vectors.

As such we construct a `StateSpaceModel` like so:
"

# ╔═╡ 9295372e-d9ad-468d-9a07-941b9cdd0d68
begin
	lg = LinearGaussian(0.5,1.0,1.0,1.0,0.0)
	lg_model = StateSpaceModel(lg,(1,1))
end

# ╔═╡ 55d9b7fe-e316-4ea9-a5e3-285d105828b5
begin
	x_sim,y_sim = simulate(lg_model,500)
	
	x_plot = plot(1:500,x_sim,ylabel="x")
	y_plot = plot(1:500,y_sim,ylabel="y")
...
# ╔═╡ 237c08fb-80c8-4727-9f70-0e8eb30e2f7c
begin
...
# ╔═╡ 931d54eb-3647-490a-af00-388c68053073
md"
### Particle Filters

Particle filters are defined in two parts: the initialization `particle_filter(⋅)`, and the propagation `particle_filter!(⋅)`.

`particle_filter(N,y,model)` generates `N` state particles from the intial distribution of the provided `model` and subsequently weights them according to the observed vector `y` at time `t=1`. This function returns a vector of states `x`, its weights `w`, and the log likelihood.

For all subsequent states updates are performed by calling `particle_filter!(x,w,y,model)`. Unlike the intialization, we have to provide state particles `x` along with their weights `w`. This function returns the resampled/propagated states `x` by operating 'in-place', as well as returning the normalized weights `w` and the log likelihood.

Using the previous model lets estimate the latent states `x` given only `y_sim`:
"

# ╔═╡ 5433288f-f441-4b65-bcb4-888b41218d8e
begin
	# pre-allocate array of expected state x̄
	x̄ = zeros(500)

	# initialize particles at t = 1
	x_particles,w_particles,logZ = particle_filter(
		Random.GLOBAL_RNG,
		512,
		y_sim[1],
		lg_model,
	...
	x̄[1] = sum(x_particles.*w_particles)

	# perform iterations t ∈ 1:500
	for t in 2:500
		logZt,w_particles,_ = particle_filter!(
			Random.GLOBAL_RNG,
			x_particles,
			w_particles,
			y_sim[t],
		...
		# find the summary statistics
		x̄[t] = sum(x_particles.*w_particles)
		logZ += logZt
	end
end

# ╔═╡ 329a0c2e-6252-4d25-9cb4-1d16aee24512
begin
	plot(1:100,x̄[1:100],ylabel="x",label="filter",xlabel="time")
	plot!(1:100,x_sim[1:100],label="simulation")
end

# ╔═╡ 55cffe5b-4f04-4b33-8511-383fa749a3b6
md"
If instead, one wants to calculate the log likelihoods to construct the log normalizing constant, we can call the function log_likelihood(⋅). This function initializes and updates the filter until it iterates through the entire vector `y`.

This function is particularly useful when calling filters into more complex estimation, such as targetting both latent states and parameters. This leads me into the next section.
"

# ╔═╡ c2a10059-e271-4fe4-8e12-0b3f810eafaa
begin
...
# ╔═╡ 232486ee-316c-4cc3-8d98-bb2d6c58feb6
md"
### SMC Objects

For parameter estimation I define an object type `SMC` which defines the characteristics of the targetted distributions as well as various algorithm characteristics.

For brevtiy, let's continue to use the above model and define a prior for θ as well as a model constructor. We use the default `SMC` constructor like so:
"

# ╔═╡ 38a802bd-5470-487f-9584-d98cceb48320
begin
	...
	smc_instance = SMC(Random.GLOBAL_RNG,512,1024,mod_func,test_prior,3,0.5)
end

# ╔═╡ 755158fc-5f57-409b-8bb1-6aabc9007963
...
# ╔═╡ 13685071-3df0-4932-b9b7-2bf23c6fea1c
md"
#### Density Tempered SMC

From [(Duan & Fulop, 2013)](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=1837772) we define the algorithm using the provided `SMC` object. Running the algorithm is done by calling the eponymous function `density_tempered(smc,y)` like so:
"

# ╔═╡ f7d2d521-961e-4169-884a-cc559ba379bc
# ╠═╡ show_logs = false
begin
	dt_smc = SMC(Random.GLOBAL_RNG,512,1024,mod_func,test_prior,3,0.5)
	density_tempered(dt_smc,y_sim[1:500])
end

# ╔═╡ 92ae6625-26d0-4f1a-bb0f-1f2aed430e5d
begin
	local θ1,θ2,θ3
	θ1 = map(m -> dt_smc.θ[m][1],1:1024)
	θ2 = map(m -> dt_smc.θ[m][2],1:1024)
	θ3 = map(m -> dt_smc.θ[m][3],1:1024)

	dt_plot1 = histogram(θ1,xlabel="A")
	dt_plot2 = histogram(θ2,xlabel="Q")
	dt_plot3 = histogram(θ3,xlabel="R")

	plot(dt_plot1,dt_plot2,dt_plot3,legend=false)
end

# ╔═╡ 7c35b6c3-ad1b-4d68-985c-b6882ce0f7ea
begin
...
# ╔═╡ 3be16bc4-1baa-45aa-9502-6560c72ac548
# ╠═╡ show_logs = false
begin
	smc²_sampler = SMC(Random.GLOBAL_RNG,512,1024,mod_func,test_prior,3,0.5)
	smc²(smc²_sampler,y_sim)
	
	for t in 2:500
	    smc²!(smc²_sampler,y_sim,t)
	end
end

# ╔═╡ d8f46fe0-a40f-4abd-aeaa-526cf727663d
begin
	θ1 = map(m -> smc²_sampler.θ[m][1],1:1024)
	θ2 = map(m -> smc²_sampler.θ[m][2],1:1024)
	θ3 = map(m -> smc²_sampler.θ[m][3],1:1024)

	θ1_plot = histogram(θ1,xlabel="A")
	θ2_plot = histogram(θ2,xlabel="Q")
	θ3_plot = histogram(θ3,xlabel="R")

	plot(θ1_plot,θ2_plot,θ3_plot,legend=false)
end
