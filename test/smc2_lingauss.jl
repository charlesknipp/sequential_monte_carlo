...
function simulate(rng::AbstractRNG,T::Int,θ::Vector{Float64})
    ...
    x[1] = rand(rng,Normal(0.0,θ[3]))

    for t in 1:T-1
        y[t]   = rand(rng,Normal(θ[2]*x[t],θ[4]))
        x[t+1] = rand(rng,Normal(θ[1]*x[t],θ[3]))
    end
...
function particle_filter(rng::AbstractRNG,N::Int,θ::Vector{Float64},y::Vector{Float64})
    ...
    x[:,1] = rand(rng,Normal(0.0,exp(θ[3])),N)

    for t in 1:T
        if t == 1
            w[:,t] = logpdf.(Normal.(θ[2]*x[:,t],exp(θ[4])),y[t])
        else
            norm_w = exp.(w[:,t-1].-maximum(w[:,t-1]))
            norm_w /= sum(norm_w)

            a = wsample(rng,1:N,norm_w,N)
            x[:,t] = rand.(rng,Normal.(θ[1]*x[a,t-1],exp(θ[3])))
            w[:,t] = logpdf.(Normal.(θ[2]*x[:,t],exp(θ[4])),y[t])
        end
    end
...
function smc²(N::Int,M::Int,y::Vector{Float64},chain_len::Int,prior)
    ...
    transition(θ,x)  = Normal.(θ[1]*x,exp(θ[3]))
    observation(θ,x) = Normal.(θ[2]*x,exp(θ[4]))
...
pred,w = smc²(100,100,y,3,prior)

pred_θ = pred[:,:,51]
mean(pred[:,:,50]*w[:,50],dims=2)
